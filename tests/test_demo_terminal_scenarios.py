...
def test_menu_keeps_scenario_keys_and_explainer_key_distinct(capsys) -> None:
    module = importlib.import_module("ask.demo_terminal_scenarios")
    module.print_menu(module.build_scenarios())
    output = capsys.readouterr().out

    assert "3. Mission-style decision options (extended)" in output
    assert f"{module.EXPLAINER_MENU_KEY}. I want to know more" in output
